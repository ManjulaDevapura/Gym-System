...
export const fetch_Users = (state, token) => {
  return (dispatch) => {
    ...
    axios
      .post("/basicData/user/get_Operators", {
    ...
    axios.post("/basicData/user/validate_Nic_User", { nic }).then((res) => {
      if (res.status === 200) {
        if (res.data) {
          axios.defaults.headers.common["authorization"] = token;
          axios
            .post("/basicData/user/add_Operators", {
              nic,
              name,
              address,
              city,
...
export const update_login = (
  ...
  return (dispatch) => {
    axios.defaults.headers.common["authorization"] = token;
    axios
      .post("/basicData/user/update_Operators", {
        id,
        nic,
        name,
        address,
...
export const delete_user = (userId, state, token) => {
  return (dispatch) => {
    axios.defaults.headers.common["authorization"] = token;
    axios.post("/basicData/user/delete_Operators", { userId }).then((res) => {
      if (res.status === 200) {
        dispatch(fetch_Users(state, token));
      }
    });
