...
import $ from "jquery";
import * as actionCreator from "../../../../actions/Basic_Data/User/Instructor_action";

const Table = lazy(() => import("./Instructor_Table"));

class User_Instructor extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    start_date = start_date.toISOString().split('T', 1);
                      ...
                      document.getElementById('add_sal').style.borderColor = null
                ...
                <div className="modal-content animate">
                      ...
                      <table>
                        ...
                        <tr>
                          ...
                          <tr>
                            <td>{t("Salary")}* :</td>
                            <td>
                              <input id="add_sal" value={this.state.add_sal} type="number" min={'0'} max={'999999999'} className="form-control" autoComplete="off"
                                onChange={(e) =>
                                  this.setState({ add_sal: e.target.value })
                                }
                              />
                            </td>
                          </tr>
                  ...
                  <div className="modal-footer">
                    <button id='add' type="button" className="btn btn-primary" onClick={() => {
                      ...
                      if((this.state.add_sal).toString().trim()===''){
                        document.getElementById('add_sal').style.borderColor = 'red'
                        valid = 0;
                        }
            if(valid===1){
                this.props.add_user(
...
const mapDispatchToProps = (dispatch) => {
  return {
    add_user: (nic, name, address, city, dob, sex, mob, sal, state, token) => {
      dispatch(actionCreator.add_user(nic, name, address, city, dob, sex, mob, sal, state, token));
    }
  };
};
