...
import * as actionCreator from "../../../../actions/Basic_Data/Member/Member_Diet_action";
const axios = require("axios");

class Member_Diet_Table extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
              ...
              <button
                ...
                onClick={() => {
                  ...
                  this.setState({
                    ...
                    view_Breakfast:row.original.breakfast,
                    view_Lunch:row.original.lunch,
                    view_Dinner:row.original.dinner,
      ...
      <Translation>
        {(t, { i18n }) => (
          <div>
            {this.props.dataSet ? (
              <ReactTable
                ...
                onFetchData={(state, instance) => {
                  this.props.fetch_Diet(state, this.props.token);
                }}
            ...
            <div id="viewDiet" className="modal fade" role="dialog">
                    ...
                    <div id="viewData" style={{ paddingLeft: 30 }}>
                      <table>
                        <tbody>
                          ...
                          <tr>
                            <td>{t("Breakfast")} :</td>
                            <td><textarea value={this.state.view_Breakfast} style={{width: 300, height:250}} disabled type="text" className="form-control" /></td>
                          </tr>
                          <tr>
                            <td>{t("Lunch")} :</td>
                            <td><textarea value={this.state.view_Lunch} style={{width: 300, height:250}} disabled type="text" className="form-control" /></td>
                          </tr>
                          <tr>
                            <td>{t("Dinner")} :</td>
                            <td><textarea value={this.state.view_Dinner} style={{width: 300, height:250}} disabled type="text" className="form-control" /></td>
                          </tr>
                          <tr>
                            <td>{t("Description")} :</td>
...
const mapStateToProps = (state) => {
  return {
    ...
    dataSet: state.r_Diet.diet_data,
    pageNo: state.r_Diet.pageNo,
    loading: state.r_Diet.loading,
    tbl_State: state.r_Diet.state,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    fetch_Diet: (state, token) => {
      dispatch(actionCreator.fetch_Diet(state, token));
    },
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(Member_Diet_Table);
