...
import $ from "jquery";
import * as actionCreator from "../../../../actions/Basic_Data/Report/Report_Membership_action";

class Report_Membership extends Component {
  constructor(props) {
    super(props);
...
this.props.fetch_Membership(this.state.startDate, endDate, this.state.empNo, this.props.token);
                  ...
                  <div style={{}}>
                    ...
                    <div id={"divcontents"} style={{ paddingTop: 0 }}>
                      ...
                      >
                        ...
                        <tbody>
                          {this.props.membRep_data
                            ? this.props.membRep_data.map(
                                (repMem, index, array) => {
                                  
            var dateObj = new Date(repMem.created).toLocaleString();
                                  return (
                                    <>
                                      <tr
                                        style={{
                                      ...
                                      >
                                        {repMem.created?dateObj.split(",", 1):''}
                                      </td>
                                        <td
                                          style={{
                                            height: 25,
...
const mapStateToProps = (state) => {
  return {
    ...
    membRep_data: state.r_RepMembership.membRep_data,
    member_data: state.r_RepMembership.member_data,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetch_Membership: (start, end, empNo, token) => {
      dispatch(actionCreator.fetch_Membership(start, end, empNo, token));
    },
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(Report_Membership);
