...
import $ from "jquery";
import * as actionCreator from "../../../../actions/Basic_Data/Report/Report_Profit_action";

class Report_Profit extends Component {
  constructor(props) {
    super(props);
...
this.props.fetch_Profit(this.state.startDate, endDate, this.props.token)
                    }}>Search</button></td>
                    <td style={{width: 50}}><span> </span></td>
                    <td style={{width: 50}}><span> </span></td>
                    <td><button
...
`;
                  ...
                  <div style={{}}>
                    ...
                    <div id={"divcontents"} style={{ paddingTop: 0 }}>
                      ...
                      >
                        ...
                        <tbody>
                            {this.props.profRep_data.length!==0
                            ? this.props.profRep_data.income.map(
                                (repExp, index, array) => {
                                  amount = amount + parseInt(repExp.value);
            var dateObj = new Date(repExp.created).toLocaleString();
                                  return (
                                    <>
                                      <tr
                                        style={{
                                      ...
                                      >
                                        {repExp.created?dateObj.split(",", 1):''}
                                      </td>
                                        <td
                                          style={{
                                            height: 25,
                            ...
                            {this.props.profRep_data.length!==0
                            ? this.props.profRep_data.expences.map(
                                (repExp, index, array) => {
                                  amount = amount - parseInt(repExp.value);
            ...
            var dateObj = new Date(repExp.created).toLocaleString();
                            ...
                            {this.props.profRep_data.length!==0
                            ? this.props.profRep_data.salary.map(
                                (repExp, index, array) => {
                                  amount = amount - parseInt(repExp.total);
            ...
            var dateObj = new Date(repExp.created).toLocaleString();
                            ...
                            {this.props.profRep_data.length!==0
                            ? this.props.profRep_data.membership.map(
                                (repExp, index, array) => {
                                  amount = amount  +parseInt(repExp.amount);
...
const mapStateToProps = (state) => {
  return {
    ...
    profRep_data: state.r_RepProfit.profRep_data,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    fetch_Profit: (start, end, token) => {
      dispatch(actionCreator.fetch_Profit(start, end, token));
    },
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(Report_Profit);
